...
#Installer version 0.74-1-yad (4 Nov 2022)
...
yad_info(){
    ...
    yad --title="Autohotspot - Setup" --center --button="OK" --text="$msg_txt"
}
...
{	
	...
	if test -f "Checklist.md5" ;then
		if ! md5sum -c --quiet Checklist.md5 ;then
			yad_info "required files in the config folder are missing or have been altered please download again"
			exit
		fi
	else
		yad_info "The file Checklist.md5 is missing from Config folder Please download the installer again"
		exit
	fi
...
hostapd_config()
{
	...
	if [ "$vhostapd" = "N" ]; then
		...
		if [ "$vhostapd" = "N" ]; then
			echo ""
			echo ""
			yad_info "Hostapd failed to install. Check there is internet access and try again"
			menu
		fi
	fi
...
dnsmasq_config()
{
	...
	if [ "$vdnsmasq" = "N" ]; then
		...
		if [ "$vdnsmasq" = "N" ]; then
		    echo ""
		    echo ""
			yad_info "dnsmasq failed to install. Check there is internet access and try again"
			menu
		fi
	fi
...
Hotspotssid()
{
	...
	if  [ ! -f "/etc/hostapd/hostapd.conf" ] ;then
		yad_info "An Access Point is not installed. No Password to change"
		menu
	else
	...
	echo ""
	yad_info "The new setup will be available next time the hotspot is started"
	menu
}

setupssid()
{
	yad_info "Searching for local WiFi connection \n \
	Add a new WiFi network or change the password for an existing one in range \n \
	For Wifi networks where only a password is required. \n \
	This will not work where a username and password is required 802.11x \n \
	If the Pi is currently in Access Point mode with a Autohotspot \n \
	then use option 6 to Force the Pi to the newly added Wifi Network"
	ct=0; j=0 ; lp=0
	wfselect=()
	...
	do
		...
		if (($j >= 5)); then #if busy 5 times exit to menu
			yad_info "WiFi Device Unavailable, cannot scan for wifi devices at this time"
			menu
			break
	...
	if [ "${#wfselect[@]}" -eq 1 ] ;then
		yad_info "Unable to detect local WiFi devices. Try again in a minute"
		menu
	fi
...
{
if [ ! -f "/etc/systemd/system/autohotspot.service" ] ;then
	yad_info "No Autohotspot script installed, unable to continue"
	menu
fi
...
get_HS_IP() #get ip address from current active hotspot script for Force Switch
{
	if [ ${Aserv: -4} = "spot" ];then #Direct
		...
		yad_info "The Autohotspot is disabled or not installed unable to force a switch."
		menu
	fi
}
...
display_HS_IP() #get ip address from current active hotspot script
{
    ...
    if [ ${Aserv: -4} = "spot" ] >/dev/null 2>&1  ;then #Direct
		ipline=($(cat /usr/bin/autohotspot | grep "ip a add")) 
		yad_info "Access Point IP Address for SSH and VNC: ${ipline[3]: :-3}" 
    elif [ ${Aserv: -4} = "potN" ] >/dev/null 2>&1 ;then #Internet
		ipline=($(cat /usr/bin/autohotspotN | grep "ip a add")) 
		yad_info "Access Point IP Address for SSH and VNC: ${ipline[3]: :-3}"
    else #Static Hotspot default IP
		yad_info "Access Point IP Address for ssh and VNC: 192.168.50.10"
    fi
}
...
go()
{
	...
	if [ "$opt" = "REM" ] ;then
		remove
		yad_info "Please reboot to complete the uninstall"
	elif [ "$opt" = "SSI" ] ;then
		setupssid
		yad_info "the new ssid will be used next time the autohotspot script is \n \
		run at boot or manually otherwise use the Force to.... option if the hotspot is active"
	elif [ "$opt" = "FOR" ] ;then
		if [ ! -f "/etc/systemd/system/autohotspot.service" ] ;then
			yad_info "No Autohotspot script installed, unable to continue"
			menu
		...
		HSssid=($(cat "/etc/hostapd/hostapd.conf" | grep '^ssid='))
		HSpass=($(cat "/etc/hostapd/hostapd.conf" | grep '^wpa_passphrase='))
		yad_info "The hotspot setup will be available after a reboot"
		display_HS_IP
	fi
}
...
menu()
{
	...
	if [ $HS == "active" ] ;then
		CURRENTNAME=$(cat /etc/hostapd/hostapd.conf | grep ssid= | head -1 | sed 's/ssid=//')
		CURRENTPASS=$(cat /etc/hostapd/hostapd.conf | grep wpa_passphrase= | sed 's/wpa_passphrase=//')
		CIP=$(ifconfig | grep -A 1 wlan0 | grep inet | awk '{ print $2 }')
		if [ -f .reboot-needed ]; then REBOOT="Reboot Needed"; fi
	fi
	CHECKF=$(ls /etc/wpa_supplicant/ | grep FORCE)
	if [ -z "$CHECKF" ] ;then
		CHECKFORCE=inactive
	else
		CHECKFORCE=active
	...
	clear
	until [ "$select" = "8" ]; do
			action=$(yad --width 400 --center --fixed --entry --title "Autohotspot" --button="gtk-ok" --button="gtk-close:1" \
			--text "Raspberryconnect.com Autohotspot installation and setup\n\n \
			For installation or switching between accesspoint types, or uninstall the access point back to standard Pi wifi.\n \
			<b>Autohospot Internet</b> Connects to a known wifi network in range otherwise automatically creates a \n \
			Raspberry Pi access point with network or internet access if an ethernet cable is connected. \n \
			Uses wlan0, eth0. on Pis 3,3plus,4\n\n \
			<b>Autohotspot Direct</b> same as above but connected devices to the access point will NOT get a \n \
			network or internet connection if an ethernet cable is connected. Rpi Zero W and RPi Zero 2\n\n \
			<b>Static Hotspot</b> = Permanent Access Point with network/internet access from eth0 for connected devices \n\n \
			Current <b>Status</b>: <i>$HS</i> :Force: <i>$CHECKFORCE</i>\n \
			Current <b>SSID: ${CURRENTNAME:-''} :Password: ${CURRENTPASS:-''}</b>\n \
			Current <b>IP of wlan0</b> for VNC/SSH: <b>${CIP:-''}</b>\n" \
			--entry-text \
			"Autohospot-Internet" "Autohotspot-Direct" "Static-Hotspot" "Remove-Autohotspot or Static Hotspot" \
			"Change/Add Wifi Network" "Force Hotspot <> Force Network" "Change-Hotspot SSID and Password" "Reboot" "Exit")
			ret=$?
			[[ $ret -ge 1 ]] && exit 0
			case $action in
				Autohospot-I*) clear ; go "AHN" ;; #Autohospot Internet
				Autohotspot-D*) clear ; go "AHD" ;; #Autohotspot Direct
				Static-H*) clear ; go "SHS" ;; #Static Hotspot
				Remove-A*) clear ; go "REM" ;; #Remove Autohotspot or Static Hotspot
				Change/*) clear ; go "SSI" ;; #Change/Add Wifi Network
				Force*) clear ; go "FOR" ;; #Force Hotspot <> Force Network
				Change-H*) clear ; go "HSS" ;; #Change Hotspot SSID and Password
				Reboot*) clear ; sudo /sbin/reboot ;;
				Exit*) clear ; exit ;;
			esac
	done
}
